...
def load_reference_data(base_dir):
    ...
    df = pd.read_csv(f"{base_dir}/01_input_data/BIOGRID-ORGANISM-Mus_musculus-4.4.246.tab3.txt", sep="\t", header=0, low_memory=False)
...
def filter_for_common_proteins(predictions, reference):
    ...
    if 'PredictiveFeature' in predictions.columns:
        predictions = predictions.rename(columns={'PredictiveFeature': 'PredictiveFeature'})
    if 'PredictiveFeature' in reference.columns:
        reference = reference.rename(columns={'PredictiveFeature': 'PredictiveFeature'})
...
def plot_aucs_per_model_per_threshold(auc_df, base_dir, network_name, int_type, title_suffix):
    ...
    colors = {
        50: '#aec7e8',  # light blue
        100: '#ffbb78', # light orange
        150: '#98df8a', # light green
        200: '#ff9896'  # light red
    }
    ...
    for i, threshold in enumerate(thresholds):
        ...
        for j, model in enumerate(model_types):
            ...
            bars = axs[0].bar(bar_positions[j] + i * bar_width, model_df['pr_auc'], 
            ...
            for bar in bars:
                ...
                axs[0].annotate(f'{height:.3f}',
                                ...
                                ha='center', va='bottom', fontsize=20, color='black', fontweight='bold', rotation=90)
    ...
    axs[0].legend(title='Min Vals', fontsize=14, title_fontsize=16, loc='upper right', frameon=True)
    ...
    for i, threshold in enumerate(thresholds):
        ...
        for j, model in enumerate(model_types):
            model_df = subset_df[subset_df['model_type'] == model]
            bars = axs[1].bar(bar_positions[j] + i * bar_width, model_df['roc_auc'], 
                      bar_width, label=threshold if j == 0 else "", color=colors[threshold])
            ...
            for bar in bars:
                ...
                axs[1].annotate(f'{height:.3f}',
                                ...
                                ha='center', va='center',
                                fontsize=20, color='black', fontweight='bold', rotation=90)
    ...
    axs[1].legend(title='Min Vals', fontsize=14, title_fontsize=16, loc='upper right', frameon=True)
    ...
    # axs[1].set_ylim(0, max_y)
        ...
        # Set y-limits to [0, 1] for both plots
...
def plot_aucs_by_model_type(auc_df, base_dir, network_name, int_type, title_suffix):
    ...
    axs[0].set_xticks(bar_positions + bar_width * (len(thresholds) - 1) / 2)
    axs[0].set_xticklabels(model_types, fontsize=12)
    axs[0].legend(title='Min Vals', fontsize=14, title_fontsize=16)
    axs[0].grid(True, linestyle='--', alpha=0.3)
    ...
    axs[1].set_xticks(bar_positions + bar_width * (len(thresholds) - 1) / 2)
    axs[1].set_xticklabels(model_types, fontsize=12)
    axs[1].legend(title='Min Vals', fontsize=14, title_fontsize=16)
    axs[1].grid(True, linestyle='--', alpha=0.3)
